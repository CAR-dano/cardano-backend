...
pipeline {
    ...
    stages {
        ...
        stage('4. E2E Tests with Test Database') {
            ...
            steps {
                withCredentials([string(credentialsId: 'db-test-password', variable: 'DB_TEST_PASSWORD')]) {
                    script {
                        env.DATABASE_URL_TEST = "postgresql://postgres:${DB_TEST_PASSWORD}@localhost:${dbTestPort}/test_db?schema=public"
