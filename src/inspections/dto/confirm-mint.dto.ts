...
export class ConfirmMintDto {
  ...
  @IsNotEmpty()
  @Length(64, 64, {
    message: 'txHash must be exactly 64 characters (Cardano transaction hash)',
  })
  @Matches(/^[a-f0-9]+$/, {
    message: 'txHash must contain only lowercase hex characters',
  })
  txHash!: string;
  ...
  @Length(56, 120, {
    message:
      'nftAssetId must be between 56 and 120 characters (Cardano asset ID)',
  })
  @Matches(/^[a-f0-9]+$/, {
    message: 'nftAssetId must contain only lowercase hex characters',
  })
  nftAssetId!: string;
}
