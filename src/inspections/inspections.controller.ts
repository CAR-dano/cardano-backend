...
import {
  ...
  ApiBody,
  ApiParam,
} from '@nestjs/swagger'; // Decorators for API documentation (Swagger/Scalar)
...
export class InspectionsController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Add new inspection record (Multipart)',
    description:
      "Submit inspection data. JSON fields (like identityDetails) must be sent as stringified JSON. Upload photos under the 'photos' field.",
  })
    ...
    schema: {
      ...
      properties: {
        ...
        identityDetails: {
          type: 'string',
          format: 'json',
          description: 'Stringified JSON for Page 1 (Identitas)',
          example: '{"namaInspektor":"Test"}',
        },
        vehicleData: {
          type: 'string',
          format: 'json',
          description: 'Stringified JSON for Page 2 (Data Kendaraan)',
        },
        equipmentChecklist: {
          type: 'string',
          format: 'json',
          description: 'Stringified JSON for Page 3 & 6 (Kelengkapan)',
        },
        inspectionSummary: {
          type: 'string',
          format: 'json',
          description: 'Stringified JSON for Page 4 (Hasil Inspeksi)',
        },
        detailedAssessment: {
          type: 'string',
          format: 'json',
          description: 'Stringified JSON for Page 5 (Penilaian)',
        },
  ...
  @ApiResponse({
    status: 201,
    description:
      'Inspection record created successfully.' /*, type: InspectionResponseDto - Add response DTO later */,
  })
  @ApiResponse({
    status: 400,
    description:
      'Bad Request (e.g., invalid file type, invalid JSON format in text fields).',
  })
  async create(
    ...
    this.logger.debug(
      'Received DTO in Controller:',
      JSON.stringify(createInspectionDto, null, 2),
    ); // Log the potentially transformed DTO
    this.logger.debug(
      'Received Files in Controller:',
      files?.photos?.map((f) => ({
        filename: f.filename,
        mimetype: f.mimetype,
        size: f.size,
  ...
  @Get()
  @ApiOperation({
    summary: 'Get all inspection records (No Pagination/Auth - Dev)',
  })
  @ApiResponse({
    status: 200,
    description:
      'List of all inspections retrieved.' /*, type: [InspectionResponseDto] */,
  }) // Add response DTO if available
  async findAll() {
  ...
  @Get(':id')
  @ApiOperation({
    summary: 'Get a specific inspection by ID (Open Access - Dev)',
  })
  @ApiParam({
    name: 'id',
    description: 'The UUID of the inspection record',
    type: String,
    format: 'uuid',
  })
  ...
  @ApiResponse({
    status: 404,
    description: 'Inspection not found for the given ID.',
  })
  async findOne(
