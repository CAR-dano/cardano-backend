...
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!requiredRoles || requiredRoles.length === 0) {
      this.logger.verbose('No specific roles required for this route. Allowing access.');
      return true;
    }
    ...
    if (!user || !user.role) {
      this.logger.warn('RolesGuard activated but user or user.role is missing from request. Denying access.');
    ...
    this.logger.verbose(`Required roles: ${requiredRoles.join(', ')}. User role: ${user.role}`);

    // 5. Check if the user's role is included in the required roles array
    const hasPermission = requiredRoles.some((role) => user.role === role);

    if (!hasPermission) {
        this.logger.warn(`User role '${user.role}' does not have permission for roles: ${requiredRoles.join(', ')}`);
        throw new ForbiddenException('You do not have permission to access this resource.');
    }

    this.logger.verbose(`User role '${user.role}' has permission. Allowing access.`);
