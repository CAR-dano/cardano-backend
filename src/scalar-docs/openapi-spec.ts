export const openApiDocument = {
    ...
    paths: {
      '/api/public/reports/{assetId}': { // Note the path parameter
        get: {
          summary: 'Retrieve Public Report Data by Asset ID',
          tags: ['Public Access'],
          parameters: [ // <-- Define the path parameter
            {
          ...
          responses: { // <-- Define at least one response
            '200': {   // HTTP Status Code (as a string)
      ...
      '/api/developer/reports': {
        get: {
          ...
          tags: ['Developer API'],
          security: [{ ApiKeyAuth: [] }],
          parameters: [ // Add query parameters if needed for filtering/pagination
