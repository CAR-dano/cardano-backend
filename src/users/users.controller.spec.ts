...
describe('UsersController', () => {
     ...
     */
    describe('findAll', () => {
        const mockUserList: any[] = [ // Use 'any' or create a partial User type for testing
            { id: '1', email: 'admin@test.com', name: 'Admin', role: Role.ADMIN, createdAt: new Date(), updatedAt: new Date() },
            { id: '2', email: 'cust@test.com', name: 'Customer', role: Role.CUSTOMER, createdAt: new Date(), updatedAt: new Date() },
        ];
        // Expected result after mapping to DTO
        const mockUserResponseList = mockUserList.map(u => new UserResponseDto(u as User));
    ...
    describe('findOne', () => {
        ...
        const mockUser: User = {
            id: testId, email: 'findone@test.com', name: 'Find Me', googleId: null,
            role: Role.REVIEWER, createdAt: new Date(), updatedAt: new Date()
        };
        ...
        it('should throw NotFoundException if user is not found', async () => {
            ...
            await expect(controller.findOne(testId)).rejects.toThrow(NotFoundException);
            expect(usersService.findById).toHaveBeenCalledWith(testId);
            expect(usersService.findById).toHaveBeenCalledTimes(1);
        });
        ...
        */
        it('should throw an error if usersService.findById fails', async () => {
            ...
            // Act & Assert
            await expect(controller.findOne(testId)).rejects.toThrow(InternalServerErrorException);
            expect(usersService.findById).toHaveBeenCalledWith(testId);
        });
    });
    ...
    describe('updateUserRole', () => {
        ...
        const mockUpdatedUser: User = {
            id: testId, email: 'updated@test.com', name: 'Updated Role', googleId: null,
        ...
        it('should update user role and return updated user as DTO', async () => {
            ...
            // Assert
            expect(usersService.updateRole).toHaveBeenCalledWith(testId, updateDto.role);
        ...
        it('should throw NotFoundException if user to update is not found', async () => {
            ...
            // Act & Assert
            await expect(controller.updateUserRole(testId, updateDto)).rejects.toThrow(NotFoundException);
            expect(usersService.updateRole).toHaveBeenCalledWith(testId, updateDto.role);
        });
        ...
        */
        it('should throw an error if usersService.updateRole fails', async () => {
            ...
            // Act & Assert
            await expect(controller.updateUserRole(testId, updateDto)).rejects.toThrow(InternalServerErrorException);
            expect(usersService.updateRole).toHaveBeenCalledWith(testId, updateDto.role);
        });
    });
    ...
    describe('disableUser', () => {
        ...
        const mockDisabledUser: User = {
            id: testId, email: 'disabled@test.com', name: 'Disabled User', googleId: null,
        ...
        it('should throw NotFoundException if user to disable is not found', async () => {
            // Arrange
            mockUsersService.setStatus.mockRejectedValue(new NotFoundException('User not found'));

            // Act & Assert
            await expect(controller.disableUser(testId)).rejects.toThrow(NotFoundException);
            expect(usersService.setStatus).toHaveBeenCalledWith(testId, false);
        });
    });
    ...
    describe('enableUser', () => {
        ...
        const mockEnabledUser: User = {
            id: testId, email: 'enabled@test.com', name: 'Enabled User', googleId: null,
            role: Role.CUSTOMER, createdAt: new Date(), updatedAt: new Date(),
        ...
        it('should throw NotFoundException if user to enable is not found', async () => {
            ...
            // Act & Assert
            await expect(controller.enableUser(testId)).rejects.toThrow(NotFoundException);
            expect(usersService.setStatus).toHaveBeenCalledWith(testId, true);
        });
    });
